...
class Panel {
    ...
    single_task_registration(data){
        ...
        let registration_select = "<div class='border-top'>履歴から予定を登録</div><div class='d-flex justify-content-around'>";
        let sq = "'";
        work_history.forEach(element => {
            registration_select+='<button class="btn btn-light" onClick="db.db_work_time_insert('+users[active_user_number].id+','+sq+data.dataset.innerdate+sq+','+sq+element[0]+sq+','+sq+element[1]+sq+')">'+element[0]+"-"+element[1]+'</button>';
        });
    ...
    single_task_changed(data){
        ...
        let registration_select = "<div class='border-top'>履歴から予定を変更</div><div class='d-flex justify-content-around'>";
        let sq = "'";
        work_history.forEach(element => {
            registration_select+='<button class="btn btn-light" onClick="db.db_work_time_update('+data.dataset.work_time_list_id+','+data.dataset.user_id+','+sq+data.dataset.date+sq+','+sq+element[0]+sq+','+sq+element[1]+sq+')">'+element[0]+"-"+element[1]+'</button>';
        });
